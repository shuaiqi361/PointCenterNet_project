...
class exkp(nn.Module):
    def __init__(self, n, nstack, dims, modules, cnv_dim=256, num_classes=80, num_codes=64):
        ...
        # codes layers
        self.codes_ = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, self.num_codes) for _ in range(nstack)])
    ...
    def forward(self, image):
        ...
        for ind in range(self.nstack):
            ...
            if self.training or ind == self.nstack - 1:
                outs.append([self.hmap[ind](cnv), self.regs[ind](cnv), self.w_h_[ind](cnv), self.codes_[ind](cnv), self.offsets[ind](cnv)])
