...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, head_conv, num_classes=7):
        self.inplanes = 64
        ...
        if head_conv > 0:
            ...
            self.offsets = nn.Sequential(nn.Conv2d(head_conv, head_conv, kernel_size=3, padding=1, bias=True),
                                         ...
                                         nn.BatchNorm2d(head_conv),
                                         nn.Conv2d(head_conv, 2, kernel_size=1, bias=True))
            self.std = nn.Sequential(nn.Conv2d(head_conv, head_conv, kernel_size=3, padding=1, bias=True),
                                         nn.ReLU(inplace=True),
                                         nn.BatchNorm2d(head_conv),
    ...
    def forward(self, x):
        ...
        out = [[self.hmap(in_cls), self.regs(inmodal_x), self.w_h_(inmodal_x), self.offsets(inmodal_x), xc_1, xc_2, xc_3, self.std(inmodal_x)]]
        return out
