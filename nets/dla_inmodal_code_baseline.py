...
class DLASeg(nn.Module):
    def __init__(self, base_name, pretrained, down_ratio, final_kernel,
        ...
        self.amodal_conv = nn.Sequential(nn.Conv2d(channels[self.first_level], head_conv, kernel_size=1, bias=False),
                                         nn.BatchNorm2d(head_conv),
                                         nn.ReLU(inplace=True),
                                         nn.Conv2d(head_conv, head_conv, kernel_size=3, padding=1, bias=False),
                                         nn.BatchNorm2d(head_conv),
                                         nn.ReLU(inplace=True))
        self.inmodal_conv = nn.Sequential(nn.Conv2d(channels[self.first_level], head_conv, kernel_size=1, bias=False),
                                         nn.BatchNorm2d(head_conv),
                                         nn.ReLU(inplace=True),
                                         nn.Conv2d(head_conv, head_conv, kernel_size=3, padding=1, bias=False),
                                         nn.BatchNorm2d(head_conv),
    ...
    def forward(self, x):
        ...
        # return [z]
        amodal_x = self.amodal_conv(y[-1])
        inmodal_x = self.inmodal_conv(y[-1])
        ...
        out = [[heatmap, regs, w_h_bbox, xc, offsets]]
        return out
