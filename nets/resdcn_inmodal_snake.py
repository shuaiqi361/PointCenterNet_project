...
class SnakeResDCN(nn.Module):
    def __init__(self, block, layers, head_conv=64, num_classes=80, snake_adj=4, dictionary=None):
        ...
        self.amodal_conv = nn.Sequential(nn.Conv2d(64, 128, kernel_size=3, padding=1, bias=True),
    ...
    def get_vertex_features(self, fmaps, poly):
        ...
        :param h: spatial dim: width and height of the last feature map, 128 x 128(512 input)
        :param w:
    ...
    def forward(self, x, inds=None, gt_center=None):
        ...
        regs_out = self.regs(inmodal_x)
        offsets_out = self.offsets(inmodal_x)
        w_h_out = self.w_h_(inmodal_x)

        in_cls = occ_feats
        hmap_out = self.hmap(in_cls)
        ...
        obj_regs = self._tranpose_and_gather_feature(regs_out.detach(), inds)
        obj_offsets = self._tranpose_and_gather_feature(offsets_out.detach(), inds)
        ...
        centered_poly = segms.permute(0, 1, 3, 2).contiguous()
...
def get_pose_resdcn(num_layers=50, head_conv=64, num_classes=80, dictionary=None):
    if dictionary is None:
        print('Please specify a dictionary in nets ...')
