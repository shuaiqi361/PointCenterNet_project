...
class exkp(nn.Module):
    def __init__(self, n, nstack, dims, modules, cnv_dim=256, num_classes=80):
        ...
        # fourier descriptor layers
        self.real_ = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, 32) for _ in range(nstack)])
        self.imag_ = nn.ModuleList([make_kp_layer(cnv_dim, curr_dim, 32) for _ in range(nstack)])
        ...
        for ind in range(self.nstack):
            ...
            if self.training or ind == self.nstack - 1:
                outs.append([self.hmap[ind](cnv), self.regs[ind](cnv), self.w_h_[ind](cnv),
                             self.real_[ind](cnv), self.imag_[ind](cnv)])
