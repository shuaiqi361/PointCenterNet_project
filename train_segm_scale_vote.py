...
from datasets.coco_segm_scale_vote import COCOSEGMCMM, COCO_eval_segm_cmm
...
parser.add_argument('--n_votes', type=int, default=121)
...
parser.add_argument('--wing_omega', type=float, default=0.1)
...
def main():
    ...
    train_dataset = Dataset(cfg.data_dir, cfg.dictionary_file,
                            ...
                            n_coeffs=cfg.n_codes, sparse_alpha=cfg.sparse_alpha, vote_len=cfg.n_votes)
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset,
                                                                    num_replicas=num_gpus,
                                                                    rank=cfg.local_rank, shuffle=True)
    ...
    val_dataset = Dataset_eval(cfg.data_dir, cfg.dictionary_file,
                               'val', test_scales=[1.], test_flip=False, fix_size=True)
            ...
            if batch_idx % cfg.log_interval == 0:
                ...
                print_log('[%d/%d-%d/%d] ' % (epoch, cfg.num_epochs, batch_idx, len(train_loader)) +
                          ...
                          (hmap_loss.item(), reg_loss.item(), w_h_loss.item(), codes_loss.item(),
                           offsets_loss.item(), shape_loss.item(), bce_loss.item()) +
