...
# from nets.resdcn_inmodal_code_active import get_pose_resdcn
from nets.resdcn_inmodal_norm_code import get_pose_resdcn

from utils.utils import _tranpose_and_gather_feature, load_model
from utils.image import transform_preds
from utils.losses import _neg_loss, _reg_loss, _bce_loss, active_reg_loss, norm_reg_loss
from utils.summary import create_summary, create_logger, create_saver, DisablePrint
from utils.post_process import ctsegm_inmodal_norm_code_decode
...
def main():
    ...
    def train(epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            hmap, regs, w_h_, offsets, codes_1, codes_2, codes_3, contour_std = zip(*outputs)
            ...
            contour_std = [_tranpose_and_gather_feature(r, batch['inds']) for r in contour_std]
            ...
            if batch_idx % cfg.log_interval == 0:
                ...
                print_log('[%d/%d-%d/%d] ' % (epoch, cfg.num_epochs, batch_idx, len(train_loader)) +
                          'Loss: hmap = %.3f reg = %.3f w_h = %.3f code = %.3f offsets = %.3f std = %.3f' %
                          (hmap_loss.item(), reg_loss.item(), w_h_loss.item(), codes_loss.item(),
                           offsets_loss.item(), std_loss.item()) +
        ...
        with torch.no_grad():
            for inputs in val_loader:
                ...
                for scale in inputs:
                    ...
                    hmap, regs, w_h_, offsets, _, _, codes, contour_std = model(inputs[scale]['image'])[-1]

                    output = [hmap, regs, w_h_, codes, offsets, contour_std]

                    segms = ctsegm_inmodal_norm_code_decode(*output,
