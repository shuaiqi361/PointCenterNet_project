...
from datasets.coco_segm_fourier import COCOSEGMFOURIER, COCO_eval_segm_fourier
from datasets.pascal import PascalVOC, PascalVOC_eval

from nets.hourglass_segm_fourier import get_hourglass
...
from utils.post_process import ctsegm_fourier_decode

# Training settings
parser = argparse.ArgumentParser(description='simple_centernet_segm_fourier')
...
def main():
    ...
    Dataset = COCOSEGMFOURIER if cfg.dataset == 'coco' else PascalVOC
    ...
    Dataset_eval = COCO_eval_segm_fourier if cfg.dataset == 'coco' else PascalVOC_eval
    ...
    def train(epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            real_ = [_tranpose_and_gather_feature(r, batch['inds']) for r in real_]
            imag_ = [_tranpose_and_gather_feature(r, batch['inds']) for r in imag_]

            hmap_loss = _neg_loss(hmap, batch['hmap'])
            reg_loss = _reg_loss(regs, batch['regs'], batch['ind_masks'])
            w_h_loss = _reg_loss(w_h_, batch['w_h_'], batch['ind_masks'])
            real_loss = norm_reg_loss(real_, batch['real'], batch['ind_masks'])
            imag_loss = norm_reg_loss(imag_, batch['imaginary'], batch['ind_masks'])
            loss = hmap_loss + 1 * reg_loss + 0.1 * w_h_loss + cfg.code_loss_weight * (real_loss + imag_loss)
            ...
            if batch_idx % cfg.log_interval == 0:
                ...
                print('[%d/%d-%d/%d] ' % (epoch, cfg.num_epochs, batch_idx, len(train_loader)) +
                      ...
                      (hmap_loss.item(), reg_loss.item(), w_h_loss.item(), real_loss.item(), imag_loss.item()) +
        ...
        with torch.no_grad():
            for inputs in val_loader:
                ...
                for scale in inputs:
                    ...
                    segms = ctsegm_fourier_decode(*output, K=cfg.test_topk)
