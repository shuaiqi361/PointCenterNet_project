...
from datasets.coco_segm_scale_snake_inmodal import COCOSEGMSNAKE, COCOSEGMSNAKEEVAL
...
from utils.post_process import ctsegm_code_shape_decode, ctsegm_scale_decode
...
def main():
        ...
        with torch.no_grad():
            for inputs in val_loader:
                ...
                for scale in inputs:
                    ...
                    hmap, regs, w_h_, offsets, _, _, codes, _, shapes_ = model(inputs[scale]['image'])[-1]
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        if (cfg.val_interval > 0 and epoch % cfg.val_interval == 0) or epoch == 3:
            stat = val_map(epoch)
            if stat > best_mAP:
                print_log('Overall mAP {:.3f} is improving ...'.format(stat))
