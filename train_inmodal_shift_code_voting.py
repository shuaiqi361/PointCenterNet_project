...
from datasets.coco_inmodal_shift_code_votes import COCOSEGMCMM, COCO_eval_segm_cmm
from datasets.kins_segm_cmm import KINSSEGMCMM, KINS_eval_segm_cmm

from nets.hourglass_segm_cmm import get_hourglass, exkp
from nets.resdcn_inmodal_code_voting import get_pose_resdcn
...
parser.add_argument('--n_votes', type=int, default=64)
parser.add_argument('--vote_loss_weight', type=float, default=0.1)
            ...
            if batch_idx % cfg.log_interval == 0:
                ...
                print_log('[%d/%d-%d/%d] ' % (epoch, cfg.num_epochs, batch_idx, len(train_loader)) +
                          ...
                          (hmap_loss.item(), reg_loss.item(), w_h_loss.item(), codes_loss.item(),
                           offsets_loss.item(), vote_loss.item()) +
