"""
The latest version as of 2020/11/20
Use the center of mass instead of the center of the bounding box
predict the distance of the center to the bounding box instead of the width and height
"""
import os
import cv2
import json
...
from utils.sparse_coding import fast_ista, check_clockwise_polygon, turning_angle_resample, \
...
class COCOSEGMSNAKE(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        centered_shapes = np.zeros((self.max_objs, self.n_vertices * 2), dtype=np.float32)
        center_offsets = np.zeros((self.max_objs, 2), dtype=np.float32)
        centers_ = np.zeros((self.max_objs, 2), dtype=np.float32)
        ...
        for k, (bbox, label, shape) in enumerate(zip(bboxes, labels, shapes)):
            ...
            if h > 0 and w > 0:
                ...
                centered_shapes[k] = (indexed_shape - mass_center).reshape((1, -1))
        ...
        return {'image': img, 'codes': codes_, 'shapes': shapes_, 'centered_shapes': centered_shapes, 'offsets': center_offsets,
                'hmap': hmap, 'w_h_': w_h_, 'regs': regs, 'inds': inds, 'ind_masks': ind_masks, 'centers': centers_,
