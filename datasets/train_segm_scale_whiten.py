...
from nets.resdcn_inmodal_scale_code_whiten import get_pose_resdcn
...
from utils.post_process import ctsegm_scale_whiten_decode

# Training settings
parser = argparse.ArgumentParser(description='inmodal_whiten_scale')
...
parser.add_argument('--n_vertices', type=int, default=128)
...
parser.add_argument('--code_loss', type=str, default='mse')
...
def main():
    ...
    with np.load(cfg.dictionary_file) as data_meta:
        cfg.dictionary = data_meta['dictionary']
        cfg.shape_mean = data_meta['mean']
        cfg.shape_std = data_meta['std'] + 1e-5
    ...
    if 'hourglass' in cfg.arch:
        ...
        model = exkp(n=5, nstack=2, dims=[256, 256, 384, 384, 384, 512],
                     modules=[2, 2, 2, 2, 2, 4],
                     dictionary=torch.from_numpy(cfg.dictionary.astype(np.float32)).to(cfg.device))
    ...
    def train(epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            dict_tensor = torch.from_numpy(cfg.dictionary.astype(np.float32)).to(cfg.device, non_blocking=True)
            dict_tensor.requires_grad = False

            outputs = model(batch['image'])
            hmap, regs, w_h_, codes = zip(*outputs)
            ...
            if cfg.code_loss == 'norm':
                ...
                codes_loss = wing_norm_reg_loss(codes, batch['codes'], batch['ind_masks'], sparsity=0.01,
            ...
            if batch_idx % cfg.log_interval == 0:
                ...
                print_log('[%d/%d-%d/%d] ' % (epoch, cfg.num_epochs, batch_idx, len(train_loader)) +
                          'Loss: hmap = %.3f reg = %.3f w_h = %.3f code = %.3f shape = %.3f' %
        ...
        with torch.no_grad():
            for inputs in val_loader:
                ...
                for scale in inputs:
                    ...
                    hmap, regs, w_h_, codes = model(inputs[scale]['image'])[-1]

                    output = [hmap, regs, w_h_, codes]

                    segms = ctsegm_scale_whiten_decode(*output,
                                                       torch.from_numpy(cfg.dictionary.astype(np.float32)).to(cfg.device),
                                                       torch.from_numpy(cfg.shape_mean.astype(np.float32)).to(cfg.device),
                                                       torch.from_numpy(cfg.shape_std.astype(np.float32)).to(cfg.device),
                                                       K=cfg.test_topk)
