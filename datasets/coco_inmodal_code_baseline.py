...
class COCOSEGMCMM(data.Dataset):
    def __init__(self, data_dir, dictionary_file, split, split_ratio=1.0, img_size=(512, 512), padding=31, n_coeffs=64,
                 n_vertices=32, sparse_alpha=0.01):
    ...
    def __getitem__(self, index):
        ...
        hmap = np.zeros((self.num_classes, self.fmap_size['h'], self.fmap_size['w']), dtype=np.float32)  # heatmaps
        w_h_ = np.zeros((self.max_objs, 2), dtype=np.float32)  # width and height of bboxes
        shapes_ = np.zeros((self.max_objs, self.n_vertices * 2), dtype=np.float32)  # gt segmentation polygons
        ...
        for k, (bbox, label, shape) in enumerate(zip(bboxes, labels, shapes)):
            ...
            shifted_shape = indexed_shape - mass_center
            ...
            if h > 0 and w > 0:
                ...
                shapes_[k] = shifted_shape.reshape((1, -1))
                center_offsets[k] = mass_center - obj_c
                codes_[k], _ = fast_ista(shifted_shape.reshape((1, -1)), self.dictionary,
                                         lmbda=self.sparse_alpha, max_iter=60)

                # shifted_poly = indexed_shape - np.array([bbox[0], bbox[1]])  # crop to the bbox
                # obj_mask = polys_to_mask([np.ndarray.flatten(shifted_poly, order='C').tolist()], bbox[3], bbox[2])
                # obj_mask = cv2.resize(obj_mask, dsize=(self.vote_vec_dim, self.vote_vec_dim), interpolation=cv2.INTER_AREA) * 1.
                # votes_[k] = obj_mask.reshape((1, -1))
...
class COCO_eval_segm_cmm(COCOSEGMCMM):
    ...
    def run_eval(self, results, save_dir=None):
        ...
        if save_dir is not None:
            result_json = os.path.join(save_dir, "inmodal_coco_baseline_results.json")
