...
def contour_mapping_loss(pred_codes, pred_shapes, gt_shapes, mask, sparsity=0.1, roll=True):
    ...
    if roll:
        ...
        loss_cmm = sum(torch.sum(
            F.l1_loss(r * mask, cmm_gt_shapes * mask, reduction='none') * 32 / scale_norm) / (mask.sum() + 1e-4) for r in pred_shapes)
    else:
        loss_cmm = sum(torch.sum(
            F.l1_loss(r * mask, gt_shapes * mask, reduction='none') * 32 / scale_norm) / (mask.sum() + 1e-4) for r in pred_shapes)
