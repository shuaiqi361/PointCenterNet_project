...
def ctsegm_scale_decode_debug(hmap, regs, w_h_, codes_, offsets_, dictionary, K=100):
    batch, cat, height, width = hmap.shape
    hmap = torch.sigmoid(hmap)
    ...
    return segmentations, codes_, offsets_.view(batch, K, 2) + torch.cat([xs, ys], dim=2).view(batch, K, 2)
...
def ctsegm_inmodal_code_decode(hmap, regs, w_h_, codes_, offsets_, dictionary, K=100):
    batch, cat, height, width = hmap.shape
    hmap = torch.sigmoid(hmap)
...
def ctsegm_scale_whiten_decode(hmap, regs, w_h_, codes_, dictionary, shape_mean, shape_std, K=100):
    batch, cat, height, width = hmap.shape
    hmap = torch.sigmoid(hmap)
    ...
    segms = torch.matmul(codes_, dictionary) * shape_std + shape_mean
    segms = segms.view(batch, K, 32, 2) * w_h_.view(batch, K, 1, 2) + \
            torch.cat([xs, ys], dim=2).view(batch, K, 1, 2)
    segmentations = torch.cat([segms.view(batch, K, -1), bboxes, scores, clses], dim=2)
